...
function SellOldProperty() {
  const router = useRouter();
  const { URI, loading, setLoading } = useAuth();
  ...
  const fetchData = async (id) => {
    try {
      const response = await fetch(URI + `/user/properties/${id}`, {
        method: "GET",
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen bg-[#F7F3FF] px-4 py-6 pt-2 sm:pt-6">
        ...
        <div className="w-full max-w-[1400px] mx-auto flex flex-col-reverse md:flex-row gap-4 sm:gap-6">
          {/* LEFT PANEL */}
          <aside className="w-full md:w-[40%] lg:w-[35%] xl:w-[30%] ">
            <div className="w-full bg-white rounded-2xl p-4 sm:p-6 py-6 h-fit lg:sticky lg:top-6 shadow sm:shadow-[0px_4px_8px_4px_#8A38F51F] mb-4">
              ...
              {step === 1 && (
                <>
                  ...
                  <div className="flex items-center gap-2 bg-[#F3EDFF] text-sm p-3 rounded-lg mb-6">
                    <FaLock color={PURPLE} />
                    Your contact details are secure and shared only with
                    verified buyers
                  </div>
...
export default SellOldProperty;
